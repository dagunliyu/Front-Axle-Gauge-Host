...
{
	/// <summary> This class handles the access to an SQL server database </summary>
	public class SQLServerDB : IDisposable,DBInterface
	{
		#region fields
		...
		private SqlDataAdapter m_daHeater;
		private DataTable m_dtMeasure;
		private SqlDataAdapter m_daMeasure;
		#endregion
		...
		#region construction
		/// <summary> Creates as SQLServerDB object </summary>
		/// <exception cref="Exception"> forwards any thrown exception </exception>
		public SQLServerDB(string dbName)
		{
			// connect to database
			...
			{
				// Connect to database
				connectionString = "Integrated Security=SSPI;Data Source=(local);Initial Catalog=" + dbName;
		...
		private void initDatabaseHeater()
		{
			...
			m_daHeater = new SqlDataAdapter(
				"SELECT * FROM Heater WHERE H_idx=(SELECT MAX(H_idx) FROM Heater)"
				,m_Connection);
			
			// Define the insert command
			m_daHeater.InsertCommand = new SqlCommand(
				"INSERT INTO Heater (H_idx , H_id , M_time , Pval , Qval , H_desc) " +
				"VALUES ( @H_idx , @H_id , @M_time , @Pval , @Qval , @H_desc )"
				,m_Connection);
			// Fill the parameters
			m_daHeater.InsertCommand.Parameters.Add("@H_idx",SqlDbType.Int,4,"H_idx");
			m_daHeater.InsertCommand.Parameters.Add("@H_id",SqlDbType.SmallInt,4,"H_id");
			m_daHeater.InsertCommand.Parameters.Add("@M_time",SqlDbType.DateTime,50,"M_time");
			m_daHeater.InsertCommand.Parameters.Add("@Pval",SqlDbType.Float,20,"Pval");
			m_daHeater.InsertCommand.Parameters.Add("@Qval",SqlDbType.Float,20,"Qval");
			m_daHeater.InsertCommand.Parameters.Add("@H_desc",SqlDbType.VarChar,10,"H_desc");
		...
		private void initDatabaseMeasure()
		{
			...
			m_daMeasure = new SqlDataAdapter(
				"SELECT * FROM Measure WHERE M_idx=(SELECT MAX(M_idx) FROM Measure)",
				m_Connection);
			
			// add the insert command
			m_daMeasure.InsertCommand = new SqlCommand(
				"INSERT INTO Measure ( H_idx , M_id , Temp , Pressure , Flow , Humidity , M_desc) " +
				"VALUES ( @H_idx , @M_id , @Temp , @Pressure , @Flow , @Humidity , @M_desc )"
				,m_Connection);
			// Fill the parameters
			m_daMeasure.InsertCommand.Parameters.Add("@H_idx",SqlDbType.Int,4,"H_idx");
			m_daMeasure.InsertCommand.Parameters.Add("@M_id",SqlDbType.SmallInt,4,"M_id");
			m_daMeasure.InsertCommand.Parameters.Add("@Temp",SqlDbType.Float,20,"Temp");
			m_daMeasure.InsertCommand.Parameters.Add("@Pressure",SqlDbType.Float,20,"Pressure");
			m_daMeasure.InsertCommand.Parameters.Add("@Flow",SqlDbType.Float,20,"Flow");
			m_daMeasure.InsertCommand.Parameters.Add("@Humidity",SqlDbType.Float,20,"Humidity");
			m_daMeasure.InsertCommand.Parameters.Add("@M_desc",SqlDbType.VarChar,10,"M_desc");
		...
		public void Insert()
		{
			if(!m_disposed)
			{
				...
				try
				{
					...
					// set identity ON, because we are managing the index of the heater table by our own.
					SqlCommand setIdent = new SqlCommand("SET IDENTITY_INSERT Heater ON",m_Connection);
					setIdent.Transaction = trans;
					setIdent.ExecuteNonQuery();
		...
		public void deleteAllTables()
		{
			if(!m_disposed)
			{
				try
				{
					...
					SqlCommand cmd = new SqlCommand("DELETE FROM Measure",m_Connection);

					// delete it
					cmd.ExecuteNonQuery();
					
					// create a "DELETE" command object for Heater table
					cmd = new SqlCommand("DELETE FROM Heater",m_Connection);
		...
		public DataSet getDataSet()
		{
			if(!m_disposed)
			{
				try
				{
					...
					SqlDataAdapter daH = new SqlDataAdapter("SELECT * FROM Heater",m_Connection);
					SqlDataAdapter daM = new SqlDataAdapter("SELECT * FROM Measure",m_Connection);
